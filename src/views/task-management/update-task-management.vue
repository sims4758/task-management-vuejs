<template>
  ...
  export default {
    props: {
      ...
      taskToEdit: {  // รับข้อมูล task ที่จะทำการแก้ไข
      type: Object,
      default: () => ({})
    }
    },
    ...
    data() {
      return {
        taskName: this.taskToEdit.name, // ใช้ข้อมูลที่รับมาจาก parent component
        taskDescription: this.taskToEdit.description,
        taskStatus: this.taskToEdit.status,
    ...
    methods: {
      async saveTask() {
        try {
          ...
          const responseSaveTask = await tasksService.updateTask(taskData);
          console.log(responseSaveTask);
          
          console.log(taskData);
          console.log('หน้า modal');
          
          
          this.$emit('edit-task', taskData); 
          this.$emit('close'); // ปิด modal
        } catch (e) {
          console.error('Error Save Task', e); // แก้ไขการแสดงผลข้อผิดพลาด
        }
      },
    },
  };
